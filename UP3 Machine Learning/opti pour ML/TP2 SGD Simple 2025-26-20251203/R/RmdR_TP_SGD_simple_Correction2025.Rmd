...
```{r question0a SOLUTION, class.source="solution"}
...
```{r question0b SOLUTION, class.source="solution"}
# On s'attend à ce que la descente de gradient trouve le minimum theta=0
...
```{r question1a SOLUTION, class.source="solution"}
...
```{r question1b SOLUTION, class.source="solution"}
...
```{r question1c SOLUTION, class.source="solution"}
...
```{r question1d SOLUTION, class.source="solution"}
...
RobbinsMonroForVaryingStepSize <- function(thetainit, Steps, alpha=1, nmax=50, nsimu=1000) {
  nbSteps <- length(Steps)
  trueTarget <- mu
  Bias <- vector(length=nbSteps)
  Variance <- vector(length=nbSteps)
  Error <- vector(length=nbSteps)
  for(i in seq(from=1, to=nbSteps)) {
    Resu <- SeveralRobbinsMonro(thetainit, Steps[i], alpha, nmax, nsimu) 
    Bias[i] <- (mean(Resu)-trueTarget)^2
    Variance[i] <- var(Resu)
    Error[i] <- mean((Resu-trueTarget)^2)
  }
  return(list(Bias=Bias, Variance=Variance, Error=Error))
}

Steps <- seq(from=0.01, to=4, length.out=100)
Resu <- RobbinsMonroForVaryingStepSize(thetainit=2, Steps=Steps, alpha=1, nmax=50, nsimu=1000)
MSE <-Resu$Bias+Resu$Variance 
plot(Steps, Resu$Bias, type='l', col='blue',log = 'y', ylim=c(1e-7, 10))
lines(Steps, Resu$Variance, type='l', col='lightblue')
lines(Steps, Resu$Error, type="l", col="purple")
lines(Steps, MSE, type='l', col='red', lty=2)

# a zoom
plot(Steps, MSE, type='l', col='red', xlim=c(0,2), ylim=c(0,5))
...
```{r question 2a - SOLUTION, class.source="solution"}
...
```{r 2b SOLUTION, class.source="solution"}
set.seed(123456)
