---
title: "TP1 Krigeage"
date: "mercredi 10 décembre 2025, variante périodique"
output:
...
NOTE: Cette version de corrigé modifie la covariance initialement proposée, à l'aide d'un noyau périodique. Les résultats sont plus flatteurs naturellement, mais on utilise une information sur la périodicité de la fonction. Dans la réalité, cela arrive souvent que l'on connaisse la périodicité. Par exemple, la pollution de l'air a une forte périodicité de 24h, du fait des heures d'affluences par exemple.
...
$$Cov(Y(x),Y(x')) = k(x,x') = \sigma^2 \exp\left(-2\frac{\sin^2(\frac{x-x'}{2})}{\theta^2}\right) $$

* Dans cette version du TP, on choisit un noyau PERIODIQUE :-) il correspond à l'équation (4.31) dans Rasmussen. Contrairement à un cosinus par exemple, il n'y a pas de corrélations négatives, il n'est donc pas parfait pour le sinus, mais on suppose qu'on ne triche pas "trop", on sait juste que la fonction est périodique.
...
k <- function(x1, x2, variance, theta) {
  return(variance * exp(-2*(sin((x1-x2)/2)^2/(theta^2))))
}
